...
from streamlit_stylable import st_stylable

# ---- Initialisation du style ----
st_stylable.init_style()
...
with st_stylable.container():
    ...
    apport = st.session_state.get("apport", 200000)
    frais_notaires = st.session_state.get("frais_notaires", 200000)
    ...
    col1, col2 = st_stylable.columns([1, 1])  # Première ligne
    ...
    col3, col4 = st_stylable.columns([1, 1])  # Deuxième ligne
    ...
    st_stylable.subheader(f"Taux d'endettement final (%) : {taux_endettement_final:,.2f}")
    st_stylable.write("---")  # Ligne de séparation pour mieux structurer la page
    col5, col6 = st_stylable.columns([1, 1])  # Troisième ligne (pour cashflow et mensualités du nouveau prêt)
...
with st_stylable.container():
    st_stylable.markdown("### Situation personnelle")
    col1, col2 = st_stylable.columns(2)
    
    with col1:
        revenu_avant = st_stylable.number_input("Revenu avant investissement (€)", min_value=1000, max_value=20000, value=3350, step=10, key="revenu_avant")
        revenu_loc_avant = st_stylable.number_input("Revenu locatif avant investissement (€)", min_value=0, max_value=20000, value=0, step=10, key="revenu_loc_avant")
        pourcentage_revenu_locatif_avant = st_stylable.slider("Pourcentage du revenu locatif pris en compte par la banque (%) - Avant investissement", 50, 100, 80)
        charge_avant = st_stylable.number_input("Charges avant investissement (€)", min_value=0, max_value=20000, value=0, step=10)
        mensualite_avant = st_stylable.number_input("Mensualité avant investissement (€)", min_value=0, max_value=20000, value=0, step=10)
...
with st_stylable.container():
    st_stylable.markdown("### Bien locatif")
    col1, col2 = st_stylable.columns(2)

    with col1:
        prix_achat = st_stylable.number_input("Prix du bien (€) - Frais d'agence compris", min_value=0, max_value=4000000, value=100000, step=1000, key="prix_achat")
        travaux = st_stylable.number_input("Travaux (facultatif) (€)", min_value=0, max_value=200000, value=0, step=1000)
        taux_frais_notaires = st_stylable.slider("Taux des frais de notaires (%)", 1, 15, 8, key="taux_frais_notaires")  # Taux des frais de notaires (par défaut 8%)
        loyer_mensuel = st_stylable.number_input("Revenu locatif mensuel (€)", min_value=0, max_value=5000, value=500, step=10, key="loyer_mensuel")
        charges_copropriete = st_stylable.number_input("Charges de copropriété (mensuel) (€)", min_value=0, max_value=1000, value=200, step=5)
        taxe_fonciere = st_stylable.number_input("Taxe foncière (annuel) (€)", min_value=0, max_value=5000, value=200, step=10)
        
    with col2:
        apport = st_stylable.number_input("Apport personnel (€)", min_value=0, max_value=4000000, value=0, step=5000)
        montant_pret = st_stylable.number_input("Montant du prêt (€)", min_value=0, max_value=4000000, value=prix_achat, step=5000)
        interet_annuel = st_stylable.number_input("Taux d'intérêt du prêt (%)", min_value=0.0, max_value=10.0, value=3.5, step=0.1)
        taux_assurance = st_stylable.number_input("Taux d'assurance (%)", min_value=0.0, max_value=4.0, value=0.3, step=0.1)
        duree_pret = st_stylable.slider("Durée du prêt (années)", 1, 30, 20)
        pourcentage_revenu_locatif = st_stylable.slider("Pourcentage du revenu locatif pris en compte par la banque (%)", 50, 100, 80)
    ...
    st_stylable.metric("Frais de notaires (€)", f"{frais_notaires:,.2f}".replace(',', ' '))
